...
| 改动 | 位置 |
|------|------|
| 写入中文前端译文（约 1.6 万条键，~71% 已译、其余回退英文） | `app\resources\ion-dist\i18n\zh-CN.json` |
| 写入中文桌面外壳译文（菜单、托盘、对话框，~94%） | `app\resources\zh-CN.json` |
...
**Q：界面大部分中文，但仍有少量英文？**
A：分两种：① 参考仓库尚未覆盖的新键（回退英文，属正常）；② i18n 没走、硬编码在 JS 里的
